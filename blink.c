...
volatile unsigned int * timer_base = (unsigned int *) 0x3F003000;
volatile unsigned int * gpio_base = (unsigned int *) 0x3F200000;

void wait(unsigned int times) {
/*	unsigned int millis = times * 1000;

	volatile unsigned int timer_low = timer_base[8 / 4];
	volatile unsigned int next_timer = timer_base[8 / 4];

	while(next_timer - timer_low < millis) {
		print_num(next_timer, uart_print);
		for (int i=0; i<0x1234;i++) {}
		next_timer = timer_base[8 / 4];
	}*/
