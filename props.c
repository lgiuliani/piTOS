...
volatile struct props8 MB_ALIGN p8 = {
	{sizeof(struct props8), 0}, {0, 4, 0}, 0, 0, 0
};
...
unsigned int props8(unsigned int tag, unsigned int selector, unsigned int value) {
	...
	p8.prop_header.property_tag = tag;
	p8.prop_header.value_size = 8; // the prop buffer size
	p8.prop_header.req_resp_ind = 4; // MSB=0=request; remaining 31 bits=prop request size
